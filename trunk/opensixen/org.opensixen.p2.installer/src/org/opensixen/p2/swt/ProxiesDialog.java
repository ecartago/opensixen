...
public final class ProxiesDialog {
		...
		typeCombo.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				if (selectedProxy == null) {
					updateStatus(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, Messages.ProxiesDialog_UnknownProxyTypeMessage, null));
				} else {
					data = selectedProxy;
					applyData();
				}
	...
	private boolean validateHostName() {
		try {
			...
			updateStatus(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, Messages.ProxiesDialog_InvalitHostMessage, null));
			return false;
		}
		return true;
	}
	...
	protected void updateStatus() {
		...
		if (hostText.getText().length() == 0) {
			updateStatus(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, Messages.ProxiesDialog_EmptyHostMessage, null));
			return;
		}

		if (userIdText.getText().length() == 0) {
			updateStatus(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, Messages.ProxiesDialog_EmptyUserMessage, null));
			return;
		}

		if (passwordText.getText().length() == 0) {
			updateStatus(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, Messages.ProxiesDialog_EmptyPasswordMessage, null));
			return;
		}
		...
		try {
			...
			if (portAsString == null || portAsString.length() == 0) {
				updateStatus(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, Messages.ProxiesDialog_EmptyProtMessage, null));
				return;
			}
			int port = Integer.parseInt(portAsString);
			if (port < 0) {
				updateStatus(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, Messages.ProxiesDialog_NegativValue, null));
				return;
			}
		} catch (NumberFormatException e) {
			updateStatus(new Status(IStatus.ERROR, Activator.PLUGIN_ID, IStatus.OK, Messages.ProxiesDialog_WrongFormat, null));
			return;
		}
