...
public class InstallDialog {
	 ...
	 */
	public InstallDialog(Shell parent) {
		createShell(parent);
	...
	private void createInstallSettingsControls() {
		...
		manualProxyTypeCheckBox.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				IProxyService proxies = (IProxyService) Activator.getService(IProxyService.class.getName());
				if (proxies != null) {
	...
	private void createShell(Shell parent) {
		shell = new Shell(parent, SWT.DIALOG_TRIM | SWT.APPLICATION_MODAL | SWT.MIN | SWT.RESIZE);
		shell.setActive();
		shell.setBounds(300, 200, 600, 200);
