...
public class AcctEditorViewer extends CFrame implements ActionListener,ChangeListener {
	...
	public AcctEditorViewer(int AD_Client_ID,int journalno){
		...
		if(journalno==0)
			createEditor(AD_Client_ID,0,0);
		else{
			createEditor(AD_Client_ID,0,0);
		}
	}
	
	public AcctEditorViewer(int AD_Client_ID, int AD_Table_ID, int Record_ID) {
		super (Msg.getMsg(Env.getCtx(), "AcctViewer"));
		createEditor(AD_Client_ID,AD_Table_ID,Record_ID);
	}
	
	public void createEditor(int AD_Client_ID, int AD_Table_ID, int Record_ID){

		setDefaultCloseOperation(DISPOSE_ON_CLOSE);
		m_data = new AcctEditorViewerData (Env.getCtx(), Env.createWindowNo(this), 
	...
	/** State Info          */
	private AcctEditorViewerData	m_data = null;
	/** Image Icon			*/
	private ImageIcon 		m_iFind = new ImageIcon(ResourceFinder.getResource("images/Find16.gif"));
...
//	private CPanel graphPanel = new CPanel();
	...
	{
		ImageIcon ii = new ImageIcon(ResourceFinder.getResource("images/InfoAccount16.gif"));
